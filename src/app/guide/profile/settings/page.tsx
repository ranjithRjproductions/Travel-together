...
export default function GuideProfileSettingsPage() {
  ...
  useEffect(() => {
    if (userProfile) {
      if (userProfile.role !== 'Guide') {
        // This is a safeguard. Traveler should never be on this page.
  ...
  }, [userProfile, reset, isUserLoading, isProfileLoading, router]);
          ...
          {isEditMode ? (
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
                <div className="flex items-center space-x-4">
                      ...
                      <div className='w-full space-y-2'>
                        <Label htmlFor="photo-upload">Update Profile Photo (Required)</Label>
                        <Input id="photo-upload" type="file" accept="image/png, image/jpeg" onChange={handleFileChange} disabled={isFormSubmitting} />
                          <p className="text-sm text-muted-foreground">
                            A clear headshot is required for verification. The description will be auto-generated by AI.
                          </p>
            ...
            <div className="space-y-4 text-sm">
                ...
                <div className="flex justify-end pt-4">
                    <Button onClick={() => router.push('/guide/profile/settings/address')}>Next Step</Button>
                </div>
            </div>
          )}
        </>
